...
def customize_model(
    model: tf.keras.Model, config: Config, tokenizer: Tokenizer
) -> tf.keras.Model:
    """
    ...
    if config["replace_recurrent_layer"]:
        logging.info(
            "Replacing recurrent layer with %s", config["replace_recurrent_layer"]
        )
        model = replace_recurrent_layer(
            model, len(tokenizer), config["replace_recurrent_layer"]
        )
    ...
    if any(
        ...
        for layer in model.layers:
            if config["freeze_conv_layers"] and (
                ...
                or layer.name.lower().startswith("residual")
            ):
                logging.info("Freezing layer: %s", layer.name)
                layer.trainable = False
            elif config["freeze_recurrent_layers"] and layer.name.lower().startswith(
                "bidirectional"
            ):
                logging.info("Freezing layer: %s", layer.name)
                layer.trainable = False
            elif config["freeze_dense_layers"] and layer.name.lower().startswith(
                "dense"
            ):
                logging.info("Freezing layer: %s", layer.name)
                layer.trainable = False
    ...
    if model_file:
        try:
            return tf.keras.models.load_model(
                model_file, custom_objects=custom_objects, compile=compile
            )
        except (TypeError, ValueError) as err:
            logging.error(
                "Error loading model. Attempting to convert the "
                "model to the new format."
            )

            # Convert the old model to the new format
            model = _convert_old_model_to_new(
                model_file, custom_objects, compile=compile
            )

            # Save the converted model
            # Rename the old model file
            if not os.path.exists(model_file + ".old"):
                logging.info("Renaming old model file to %s", model_file + ".old")
...
def _convert_old_model_to_new(
    model_file: str, custom_objects: dict, compile: bool = True
) -> tf.keras.Model:
    """
    ...
    def correct_axis(layer_config):
        if isinstance(layer_config, dict):
            if layer_config.get("class_name") == "BatchNormalization" and isinstance(
                layer_config.get("config", {}).get("axis"), list
            ):
    ...
    with warnings.catch_warnings():
        ...
        model.load_weights(
            os.path.join(temp_dir, "model.weights.h5"), skip_mismatch=True
        )
...
def load_or_create_model(
    config: Config, custom_objects: Dict[str, Any]
) -> tf.keras.Model:
    """
...
def build_predefined_model(config: Config, model_key: str) -> tf.keras.Model:
    ...
    model = model_generator.generate_model(
        ...
        model_name=config["model_name"] if config["model_name"] else model_key,
    )
