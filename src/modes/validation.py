...
def perform_validation(config: Config,
    ...
    """
    logging.info("Performing %s...", 'validation')

    dataset = data_manager.datasets['validation']
    tokenizer = data_manager.tokenizer
    wbs = setup_word_beam_search(
        config, tokenizer.token_list) if config["corpus_file"] else None

    metrics_calculator = MetricsCalculator(
        config, log_results=True, maxsize=100)
    ...
    output_statistics(metrics_calculator, config, 'validation', wbs)
